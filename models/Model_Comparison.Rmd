...
m3.c.jointPlot = m3.c.matched %>%
  gather(study, listener.pred, listener.p, e6.listener.p)
m3.c.jointPlot$study = ifelse(m3.c.jointPlot$study == "e6.listener.p", "e6", "e11")
qplot(stars, listener.pred, col=degree, shape=study,
      data=m3.c.jointPlot,
      main=paste("m3.c\nalpha: ", alphas[11],
                 "\nNormalized = ", T,
                 ylab="Posterior p(rating | word)")) +
...
# Store plot 2 - both e6 and e11 data
m3.d.jointPlot = m3.d.matched %>%
  gather(study, listener.pred, listener.p, e6.listener.p)
m3.d.jointPlot$study = ifelse(m3.d.jointPlot$study == "e6.listener.p", "e6", "e11")
qplot(stars, listener.pred, col=degree, shape=study,
      data=m3.d.jointPlot,
      main=paste("m3.d\nalpha: ", alphas[12],
                 "\nNormalized = ", T,
...
# Store plot 2 - both e6 and e11 data
m4.c.jointPlot = m4.c.matched %>%
  gather(study, listener.pred, listener.p, e6.listener.p)
m4.c.jointPlot$study = ifelse(m4.c.jointPlot$study == "e6.listener.p", "e6", "e11")
qplot(stars, listener.pred, col=degree, shape=study,
      data=m4.c.jointPlot,
      main=paste("m4.c\nalpha: ", alphas[15],
                 "\nNormalized = ", T,
...
# Store plot 2 - both e6 and e11 data
m4.d.jointPlot = m4.d.matched %>%
  gather(study, listener.pred, listener.p, e6.listener.p)
m4.d.jointPlot$study = ifelse(m4.d.jointPlot$study == "e6.listener.p", "e6", "e11")
qplot(stars, listener.pred, col=degree, shape=study,
      data=m4.d.jointPlot,
      main=paste("m4.d\nalpha: ", alphas[16],
                 "\nNormalized = ", T,
...
ggplot(barchart.d, aes(x=model, y=cor, fill=study)) + 
  ...
  # geom_text(aes(x=model, y=as.numeric(cor) + 0.025, label=round(cor, 3)))
```

In particular, we first start to see a pretty 
...
This improvement is particularly clear when we look at e11 data:
```{r}
qplot(stars, listener.p, col=degree, 
      data=m3.c.matched,
      main=paste("m4.c\nalpha: ", alphas[11],
                 "\nNormalized = ", T,
                 "\nCorr = ", performance.output[11, 3]),
      ylab="Posterior p(rating | word)") + 
  ylim(0, 1) +
  facet_wrap(~scale) + 
  geom_line(aes(y = pred), lty = 4)
...
1) In general model performance is greater (correlation) for e11 data. This may be due in part to less extreme literal listener evaluations in e11 (see `RSA+AmbiguousSemantics.Rmd` for a more detailed analysis for literal listener experiment differences). Our best model performance occurs with e11 and the model is m4.c, correlation = `r as.numeric(performance.output[15, 3])` (see plot below):
```{r}
qplot(stars, listener.p, col=degree, 
      data=m4.c.matched,
      main=paste("m4.c\nalpha: ", alphas[15],
