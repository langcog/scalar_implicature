...
performance.output = data.frame(model=rep(NA, 8), norm.cor=rep(NA, 8))
alphas = rep(NA, 8)

# Model 1 - Entailment only model
# --------------------------------
# a)
# Normalized = T
# alpha tune = F
...
performance.output[1, ] = c("M1_noFit", round(cor(m1.noFit$listener.p, m1.noFit$pred), 5))
# b)
# Normalized = F
# alpha tune = F
alphas[1] = 1
...
# ----------------------------------
alphas[2] = tune.alpha(data.partial, normalize=F)
m1.fit = data.partial %>%
  do(run.partial(., alpha = alphas[2]))
performance.output[2, ] = c("M1_fit", round(cor(m1.fit$listener.p, m1.fit$pred), 5))
...
performance.output[3, ] = c("M2_noFit", round(cor(m2.noFit$listener.p, m2.noFit$pred), 5))
...
performance.output[4, ] = c("M2_fit", round(cor(m2.fit$listener.p, m2.fit$pred), 5))

# Model 3 - Full, no alpha tuning
# -------------------------------
alphas[5] = 1
m3.noFit = data.full %>%
  do(run.full(., alpha = alphas[5], addMid=F, normalize=F))
...
performance.output[5, ] = c("M3_noFit", round(cor(m3.noFit.matched$e6.listener.p, m3.noFit.matched$pred), 5))
