...
`m2:` Entailment + generic None models (i.e. "some_all" scale includes "some", "all", "none")

`m3:` Full model with empirical alternatives (i.e. "some_all" scale includes "some", "all", "most", "none")

`m4:` Entailment + empirical alternatives + additional fitted scalar (i.e. "some_all" scale includes "some", "all", "most", "none", "mid")
...
We see fairly steady model improvement with the addition of salient alternatives. Let's focus on the normalized family of models (1a, 1c, ... , 4a, 4c) - I explain why this might be a good idea below, but I'm not totally convinced. Probably the most important comparisons to pay attention to is the jump in correlation values when we first introduce empirically derived alternatives (models m3). Comparing m2 models (entailment + generic None) and m3 models (empirically derived alternatives) we see jumps of +0.23 for r values with alpha tuning. See the plots below:
```{r}
m2.c.plot
m3.c.plot
```
...
1) In general model performance is greater (correlation) for e11 data. This may be due in part to less extreme literal listener evaluations in e11 (see RSA+AmbiguousSemantics for a more detailed analysis for literal listener experiment differences). Our best model performance occurs with e11 and the model is m4.c, correlation = `r round(performance.outpu[15, 3], 5)`.

2) Non-normalized models seem to be more sensitive to alpha tuning as the number of alternatives increases (see m4.d and m3.d with alpha = 8). I'm inclined to stick with normalized model results because of this - we're interested in whether we see model improvement from the addition of salient alternatives - not from tuning hyperparameters. We tend to see more model improvement with alternatives when we're looking at normalized models (see `m3.c` vs `m4.c`).

3) We may want to explore the the idea of unique alphas for a given scale (would need to change how we've written RSA), for example using scale entropy measures.
```{r overall_perf}
