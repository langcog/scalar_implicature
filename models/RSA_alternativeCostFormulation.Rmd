...
normNegSalienceCosts <- function(saliences) {
  ...
  for (s in scales) {
    saliences[saliences$scale == s, ]$cost <-
      (-1 * saliences[saliences$scale == s, ]$newCount) / NUM_ALTERNATIVES_E9
    saliences[saliences$scale == s, ]$cost <-
      -1 * normedVec(saliences[saliences$scale == s, ]$cost)
  }
...
```{r}
# Run analyses
symmetricScalerun <- completeModelRun(speakerL12, listener, unifPriors, emptyCosts5, numAlts = 5)
symmetricScalerun[[4]]
negAltsRun <- completeModelRun(speakerL10, listener, unifPriors, emptyCosts4, numAlts = 4)
negAltsRun[[4]]
entailmentRun <- completeModelRun(speakerL08, listener, unifPriors, emptyCosts2, numAlts = 2)
entailmentRun[[4]]
