...
runFull <- function(data, calcFn = 1, alpha = 1, numAlts = 5) {
  results <-
    data.frame(scale = c(),
               stars = c(),
               degree = c(),
...
salienceCosts <- read.csv("~/Desktop/Projects/scalar_implicature/models/saliences.csv")
for (s in scales) {
  salienceCosts[salienceCosts$scale == s, ]$cost <-
    -1 * normedVec(salienceCosts[salienceCosts$scale == s, ]$cost)
}
...
# run fns()
checkSaliences()
tuneAlphas(modelData = data, humanDf = listener)
```
...
# -----------------------
matchModelHumanData <- function(predDf, listenerDf, study) {
...
# mergedData <- matchModelHumanData(test, listener)

# Alpha Tuning - maximixize correlation
# -------------------------------------
tuneAlphas <- function(modelData, humanDf, alphas = seq(1, 12), numAlts = 5){
  sapply(alphas, function(i) {
    predDf <- runFull(modelData, calcFn = 2, alpha = i, numAlts = numAlts)
    mergedData <- matchModelHumanData(predDf, listener)
    cor(mergedData$preds, mergedData$listener.p)
  })
}
tuneAlphas(modelData = data, humanDf = listener)

checkSaliences <- function() {
  predDf <- runFull(data, calcFn = 2, alpha = 1, numAlts = 5)
  mergedData <- matchModelHumanData(predDf, listener)
  cor(mergedData$preds, mergedData$listener.p)
}
checkSaliences()
...
correlationScatterPlot <-
  ggplot(mergedData, aes(x = listener.p, y = preds, col=stars)) +
  geom_point() + 
