...
completeModelRun <- function(speaker, listener, priors, costs, numAlts) {
  ...
  # plots
#   predPlot(combinedDf)
#   correlationScatterPlot(combinedDf)
...
Aggregate analysis
```{r}
cors <- data.frame(cors = c(symmetricScalerun[[4]], negAltsRun[[4]], entailmentRun[[4]]),
                   m = c("full", "neg_alts", "entailment"))
ggplot(cors, aes(x=reorder(m, cors), y = cors)) +
  ...
  guides(colour=FALSE) +
  geom_text(aes(x=m, y=as.numeric(cors) + 0.025, label=round(cors, 3)))
```
