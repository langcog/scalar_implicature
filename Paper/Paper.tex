...
secondary hypothesis) we ran a mixed effects model regressing
compatibility judgments on scale, number of stars and experimental
...
needed a way of selecting plausible alternatives. We addressed this
issue by adopting a modified cloze task to elicit empirical alternatives
for the entailment items we used in Experiment 1a and 3a. This design
was inspired by Experiment 3 of Tiel (2014).
...
expanded our alternative set from only entailment items (Experiments 1a,
3a) to four alternatives (Experiments 1b, 3b) by including the top two
alternatives elicited for each scale in Experiment 2. For example, the
top two alternatives for the \emph{liked}/\emph{loved} scale were
\emph{hated} and \emph{disliked} and were included in four-alternatives
model runs. Similarly, neutral items (e.g. \emph{okay} in the
\emph{good}/\emph{excellent} scalar family) were chosen from among the
set of alternatives elicited in Experiment 2. Alternative set
distributions for the other scalar families (e.g.
\emph{good}/\emph{excellent}, \emph{memorable}/\emph{unforgettable})
were similarly long-tailed. See Figure \ref{fig:allScalesTable} for the
...
generic negative alternative
\footnote{literal semantics for the generic negative alternative were not generated empirically, but serve as a baseline "none" style semantics in which the scalar item is only compatible with 1 star},
