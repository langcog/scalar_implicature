...
ggplot(speakers, 
       aes(x = stars, y = speaker.p, col = scale, lty = study)) + 
  geom_line() + 
  # facet_grid(.~ scale) + 
...
```{r exp2_altsPlot_likedLoved, fig.pos = "t", fig.align='center', set.cap.width=T, num.cols.cap=2, fig.cap = "Combined counts for participant generated alternatives for 'liked \ loved' scale. Participants were shown a target scalar (either liked or loved) and asked to give three alternatives."}
...
We recruited 100 participants from AMT, 50 for each condition. Data for `r 50 - length(unique(exp3a$workerid))` participants in the two-alternative condition were excluded after participants either failed to pass two training trials or reported a non-English native language, leaving a total sample of `r length(unique(exp3a$workerid))` participants. In the four-alternative condition, data from `r 50 - length(unique(exp3b$workerid))` participants was thrown out after participants either failed to pass two training trials or were not native English speakers, leaving a total sample of `r length(unique(exp3b$workerid))` participants.
...
To test whether there were between experiment differences in pragmatic judgments, we ran a mixed effects model, regressing pragmatic judgements on scale and experiment with subject and word level random effects. Results indicate there were no significant differences between Experiment 3a and 3b for $b =$ `r round(sumGlm2$coefficients[2], 2)`, $t($ `r round(sumGlm2$coefficients[14], 2)` $) =$ `r round(sumGlm2$coefficients[20], 2)`, $p =$ `r round(sumGlm2$coefficients[26], 2)` and the addition of our experiment predictor did not significantly improve model fit when compared to a model without the experiment variable using ANOVA $\chi^2($ `r anovaResults$"Chi Df"[2]` $) =$ `r round(anovaResults$Chisq[2], 2)`, $p =$ `r round(anovaResults$"Pr(>Chisq)"[2], 2)`.
...
names(tableCors) <- c("Model", "Exp. 3a", "Exp 3b")
tableCors <- tableCors[c(1, 3, 2),]
tab <- xtable(tableCors, caption = "Model fit measured by correlation coefficients")
